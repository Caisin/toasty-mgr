...
动态查询参数、Expr、count、all 和可选分页入口，并使用 `bon` 提供链式 builder。
...
追加唯一键。分页规格可以不声明排序，但调用方需要自行承担跨页顺序不稳定的风险。
...
操作、排序白名单、默认排序和可选分页区块同时提供精确 span 错误。函数式过程宏只分析
...
暴露的条件、排序、执行出口和可选分页。链式请求 setter 仍委托给 `bon::Builder`，不
