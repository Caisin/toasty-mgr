...
但无法检查 Toasty 模型中未出现在查询规格里的数据库字段。新增数据库字段时不会触发
任何提示，也无法可靠区分关系字段与数据库字段，或对嵌入和自定义类型给出字段级 span
错误。

本设计拒绝声明式宏单独承担 query builder 扩展。`#[derive(TcQuery)]` 负责数据库字段
完整覆盖和固有方法，并排除关系字段；`tc_query_spec!` 只负责某个请求允许暴露的条件、
排序、执行出口和可选分页。链式请求 setter 仍委托给 `bon::Builder`，不自行实现
typestate builder。
